        ...
        <div className="w-full max-w-screen-xl mx-auto px-4 md:px-6">
          <div className="flex items-center justify-between h-[90px]">
            ...
            <div className="hidden md:flex items-center gap-10">
              <NavItem href="/services/train-delivery">Menu</NavItem>
              <NavItem href="/about-us">About Us</NavItem>

              {/* SERVICES */}
              <div className="relative group">
                <button className="flex items-center gap-1 text-gray-700 hover:text-orange-600 transition">
                  <span className="relative after:block after:w-0 after:h-[2px] after:bg-orange-500 after:transition-all after:duration-300 group-hover:after:w-full">
                    Services
                  </span>
                  <ChevronDown size={16} />
                </button>

                <div className="absolute top-10 left-0 w-48 bg-white border shadow-md rounded-md 
                  opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 py-2">
                  <DropdownItem href="/bulk-order">Bulk Order in Train</DropdownItem>
              ...
              <div>
                ...
                {servicesOpen && (
                  <div className="ml-4 space-y-2">
                    <MobileItem href="/bulk-order" setIsOpen={setIsOpen}>
                      Bulk Order in Train
                    </MobileItem>
...
function NavItem({ href, children }) {
  return (
    <Link href={href} className="text-gray-700 hover:text-orange-600 transition">
      {children}
    </Link>
  );
}
...
function DropdownItem({ href, children }) {
  return (
    <Link href={href} className="block px-4 py-2 hover:bg-orange-50">
      {children}
    </Link>
  );
}
